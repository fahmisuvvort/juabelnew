<template>
  <q-page>
    ...
    <!-- <div class="border q-pa-md row item-center q-gutter-x-sm">
      <div class="col">
        ...
        <q-btn outline label="Reset" @click="getProducts" color="primary"></q-btn>
      </div> -->
  ...
  <div class="flex justify-center q-py-md">
    <q-btn label="Load More..." outline color="primary" @click="paginate" v-if="products.count > products.data.length"></q-btn>
  </div>
    </div>
    </template>
    <template v-else >
...
export default {
  ...
  computed: {
    ...mapState({
      products: state => state.seller.products,
      loading: state => state.loading
    }),
  },
  methods: {
    ...mapActions('seller', ['getProducts', 'productDelete', 'searchProductSeller', 'getPaginateProduct']),
    searchProduct() {
      this.$store.commit('SET_LOADING', true)
      this.$refs.input.blur()
      this.searchProductSeller(this.search).then(response => {
        if(response.status == 200) {
          this.$store.commit('seller/SET_SELLER_PRODUCTS', response.data)
        }
      }).finally(() => {
    ...
    paginate() {
      this.getPaginateProduct({ skip: this.products.data.length, filter: this.filter })
    }
  },
  mounted() {
